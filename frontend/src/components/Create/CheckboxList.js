...
class CheckboxList extends React.Component {
  render() {
    ...
    const listItemMainServices = services.map((item) => (
      <CheckboxListItem
        {...item}
        key={item.serviceId}
    ...
    const listItemAddServices = addServices.map((item) => (
      <CheckboxListItem
        {...item}
        key={item.serviceId}
    ...
    return (
      <div className={s.services}>
        <div className={s.titleBox}>
          ...
          <span className={s.isRequiered}> * </span>
        </div>
        <div className={s.titleBox}>
          Вы также можете указать дополнительные услуги за дополнительную
          стоимость{' '}
        </div>
        <div>
          <CheckList>{listItemMainServices}</CheckList>
        </div>
        <div>
          <CheckList>{listItemAddServices}</CheckList>
        </div>
      </div>
    );
  }
