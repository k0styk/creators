...
class StoreWrapper extends Component {
  ...
  render() {
    ...
    if (
      authStatus === authStatusEnum.IS_AUTHENTICATED &&
      RouterStore.authRoutes.includes(name)
    ) {
      return <Redirect to="/" />;
    }
    if (
      authStatus === authStatusEnum.IS_NOT_AUTHENTICATED &&
      RouterStore.privateRoutes.includes(name)
    ) {
