...
class FilterStore {
    ...
    @computed get filterIsEmpty() {
        return (
            ...
            !this.selectedType &&
            !this.time &&
            !Object.values(toJS(this.price)).filter(Boolean).length &&
            !this.fastFilter
        );
    }
    ...
    @action setTimeObject = (timeto, timefrom) => {
        this.time = this.times.find(
            ({ to, from }) =>
                (Number(timeto) &&
                    Number(timefrom) &&
                    Number(to) === Number(timeto) &&
                    Number(from) === Number(timefrom)) ||
                Number(timeto) === Number(to) ||
                Number(timefrom) === Number(from)
        );
    };
